...
class BBoxRayHelper:
    def __init__(self, dataset_config, instance_id):
        ...
        assert self.dataset_name in ["udc_dataset_toydesk", "udc_dataset_scannet_multi"]

        if self.dataset_name == "udc_dataset_scannet_multi":
            print(self.conf.keys())
            self.scene_id = self.conf["scene_id"]
            self.read_bbox_info_scannet()
        elif self.dataset_name == "udc_dataset_toydesk":
