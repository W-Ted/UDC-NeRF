...
class ToydeskDataset(Dataset):
    ...
    def get_instance_mask(self, instance_path, instance_id):
        ...
        if isinstance(instance_id, int):
            ...
            if self.conf["erode_instance_mask"]: # false
                print('Erode instance mask: ', self.conf["erode_instance_mask"])
                mask = ndimage.binary_erosion(mask, structure=np.ones((3,3))).astype(mask.dtype)
                mask = ndimage.binary_dilation(mask, structure=np.ones((7,7))).astype(mask.dtype)
    ...
    def read_meta(self):
        ...
        if self.split == "train":
            ...
            split_inds = np.loadtxt(
                os.path.join(self.conf["split"], "test.txt")
            ).tolist()
